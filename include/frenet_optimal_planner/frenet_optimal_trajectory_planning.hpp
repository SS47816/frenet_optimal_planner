...
public:
  ...
  FrenetOptimalTrajectoryPlanning(Setting settings);
  ...
  ResultType generateReferenceCurve(const frenet_optimal_planner::common::Map& map);

  // Plan for the optimal trajectory
  std::vector<frenet_optimal_planner::common::FrenetPath> frenetOptimalPlanning(frenet_optimal_planner::common::Spline2D& cubic_spline,
                                                             const frenet_optimal_planner::common::FrenetState& frenet_state,
...
private:
  ...
  std::vector<frenet_optimal_planner::common::FrenetPath> generateFrenetPaths(const frenet_optimal_planner::common::FrenetState& frenet_state, 
  ...
  std::vector<frenet_optimal_planner::common::FrenetPath> calculateGlobalPaths(std::vector<frenet_optimal_planner::common::FrenetPath>& frenet_paths_list,
                                                            frenet_optimal_planner::common::Spline2D& cubic_spline);

  // Check for collisions and calculate obstacle cost
  bool checkPathCollision(const frenet_optimal_planner::common::FrenetPath& frenet_path, const autoware_msgs::DetectedObjectArray& obstacles, const std::string& margin);

  // Check for vehicle kinematic constraints
  std::vector<frenet_optimal_planner::common::FrenetPath> checkPaths(const std::vector<frenet_optimal_planner::common::FrenetPath>& frenet_paths_list,
                                                  const autoware_msgs::DetectedObjectArray& obstacles, int path_size);

  // Select the best paths for each lane option
  std::vector<frenet_optimal_planner::common::FrenetPath> findBestPaths(const std::vector<frenet_optimal_planner::common::FrenetPath>& frenet_paths_list);

  // Select the path with the minimum cost
  frenet_optimal_planner::common::FrenetPath findBestPath(const std::vector<frenet_optimal_planner::common::FrenetPath>& frenet_paths_list, int target_lane_id);
};
